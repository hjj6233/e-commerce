<template>
	<view class="register">
		...
		<view class="content">
			...
			<!-- 主体 -->
			<view class="main">
				<!-- <wInput v-model="realname" type="text" placeholder="用户名"></wInput> -->
				<wInput v-model="phoneData" type="number" maxlength="11" placeholder="请输入手机号"></wInput>
				<wInput v-model="password" type="password" :isShowPass="true" placeholder="密码"></wInput>
				<wInput v-model="password_repetition" type="password" :isShowPass="true" placeholder="确认密码"></wInput>
				<wInput v-model="verCode" type="number" maxlength="6" placeholder="请输入验证码" isShowCode ref="runCode" @setCode="sendMsg1()"></wInput>
				<!-- <wInput v-if="show" v-model="invitation" type="number" isShowGet ref="setNumberCode" @setNumberCode="isShowGet()" placeholder="请填写邀请码"></wInput> -->
			</view>
			<wButton text="登 录" :rotate="isRotate" @click.native="startReg1()"></wButton>
			<!-- 底部信息 -->
			<view class="footer">
	...
	export default {
		...
		methods: {
			...
			startReg1() {
				...
				// if (this.verCode != this.getVerCode) {
				// 	uni.showToast({
				// 		icon: 'none',
				// 		position: 'bottom',
				// 		title: '验证码不正确'
				...
				this.$Request.postP("/api/tao-pie/v1/site/register", {
					...
					if (res.message === 'OK') {
						...
						this.$queue.setData("userId", res.data.member.id);
						this.$queue.setData("mobile", this.phoneData);
						this.getUserInfo1(res.data.access_token);
					} else {
						this.isRotate = false;
						// uni.showModal({
						// 	showCancel: false,
			...
			getUserInfo1(token) {
				this.$Request.postP("/api/tao-pie/v1/site/login",{
					mobile: this.phoneData,
					group: this.group,
					password: this.password
				}).then(res => {
